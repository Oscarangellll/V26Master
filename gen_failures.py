...
def gen_failures(seed, wf, maintenance_categories):
    ...
    p = np.array([m.failure_rate / 365 for m in maintenance_categories], dtype=float) #bestem om 360 eller 365
    if p.sum() > 1.0:
    ...
    draws = rng.multinomial(int(50), p_all, size=360)
    ...
    fdf = pd.DataFrame({
        ...
        "subcomponent": np.tile([m.name for m in maintenance_categories], 360),
